...
for(var i=orderedBlocks.length-1;i>=0;i--){if(orderedBlocks[i].start>row[k].stop){orderedBlocks[i].y=y;row.push.apply(row,orderedBlocks.splice(i,1));k++}}rows.push(row)}}/**
    ...
    */},{key:'_drawTrack',value:function _drawTrack(i){var _this3=this;var obj=this,datum=this.data.tracks[i],name=datum.genus+' '+datum.species,c=this.options.colors(name);// create the track's rows of blocks
this._blocksToRows(datum.blocks);// create the track
var selector='macro-'+i.toString(),track=this.viewer.append('g').attr('data-macro-track',i.toString()).attr('data-chromosome',datum.chromosome).attr('data-genus-species',datum.genus+' '+datum.species);track.offset=0;// create the track's blocks
var blocks=track.selectAll('block').data(datum.blocks).enter().append('g').style('cursor','pointer').on('mouseover',function(b){obj.block=this;obj._beginHover(d3.select(this))}).on('mouseout',function(b){obj.block=undefined;obj._endHover(d3.select(this))}).on('click',function(){_this3.options.blockClick()});// help for generating points
...
var tips=blocks.append('text').attr('class','synteny-tip').attr('text-anchor','end').text(function(b){return b.block}).attr('data-x',function(b){var x1=b.start,x2=b.stop;return x1+(x2-x1)/2}).attr('data-y',function(b){return(obj._BLOCK_HEIGHT+obj._PAD)*(b.y+1)}).attr('transform',function(b){var tip=d3.select(this),x=obj.scale(tip.attr('data-x')),y=tip.attr('data-y');return'translate('+x+', '+y+')'});// how the blocks are resized
